...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showNotification(err.response?.data?.error || 'Login failed. Please try again.', false);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="h-screen flex overflow-hidden">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center bg-white px-4 sm:px-6 py-6 overflow-y-auto">
        <div className="w-full max-w-md">
          ...
          {/* Form */}
          <form onSubmit={handleSubmit} className="space-y-4" noValidate autoComplete="off" data-lpignore="true">
            {/* Email */}
            <div>
              <label className="block text-sm font-medium text-hosanna-black mb-1.5">
                Email
              </label>
              <div className="relative">
                <Mail className="absolute left-3.5 top-1/2 -translate-y-1/2 w-5 h-5 text-hosanna-gray" />
                <input
                  type="email"
                  name="email"
                  value={form.email}
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="block text-sm font-medium text-hosanna-black">
                  Password
                </label>
                <button type="button" className="text-sm text-hosanna-red hover:text-hosanna-red-dark font-medium transition-colors">
                  Forgot password?
                </button>
              </div>
              <div className="relative">
                <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-5 h-5 text-hosanna-gray" />
                <input
            ...
            <div className="flex items-center">
              <input
                ...
                data-lpignore="true"
                className="w-4 h-4 text-hosanna-red border-gray-300 rounded focus:ring-hosanna-red accent-hosanna-red"
              />
              <label htmlFor="remember" className="ml-2 text-sm text-hosanna-gray cursor-pointer">
                Remember me
              </label>
            </div>
          ...
          {/* Sign up link */}
          <p className="mt-4 text-center text-sm text-hosanna-gray">
          ...
          {/* Footer links */}
          <div className="mt-4 flex items-center justify-center gap-6 text-xs text-hosanna-gray">
