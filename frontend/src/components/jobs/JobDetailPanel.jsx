...
/**
 * Which buttons to show based on current status + user role.
 * Matches backend STATUS_TRANSITIONS from constants.js exactly.
 *
 * Flow: TENTATIVE → CONFIRMED → ASSIGNED → DISPATCHED → IN_PROGRESS → COMPLETED → BILLED
 *   - After Admin assigns a tech, the job goes to ASSIGNED (visible to Manager).
 *   - Manager reviews and dispatches with a mandatory note → DISPATCHED (now visible to Tech).
 */
const NEXT_ACTIONS = {
...
export default function JobDetailPanel({ job: initialJob, userRole, onClose, onStatusChanged, onAssignClick }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      {/* Slide-in Panel */}
      <div className="relative bg-white w-full max-w-lg shadow-2xl h-full overflow-y-auto animate-slide-in">
        {/* Header */}
        ...
        <div className="px-6 py-5 space-y-6">
          ...
          {canPerformAction && (
            <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
              ...
              {!nextAction.isAssign && (
                <>
                  {/* Notes area — always visible when notes are required */}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {/* Only show the toggle button when notes are NOT required (they're always visible when required) */}
                    {!nextAction.requiresNotes && (
                      <button
              ...
              {/* Assign action → opens the assign modal */}
              {nextAction.isAssign && (
                <button
          ...
          <div>
            ...
            <div className="space-y-2.5">
              ...
              {job.address && <InfoRow icon={MapPin} label="Address" value={job.address} />}
            </div>
          </div>
